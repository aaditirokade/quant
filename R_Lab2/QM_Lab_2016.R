#########################################################
#####  R Lab 2: Charts and Graphs -- 2016 GSS Data  #####
#########################################################
...
GSS2016 <- read_csv("/Users/aaditirokade/Desktop/Quant/R_Lab_2/GSS_2016.csv")
...
unique(GSS2016$polviews) 

  count(GSS2016, polviews) %>%              
...
count(GSS2016, race)           #gives a count for each race

# We then use the numebers from the output
# above to create a simple pie chart
slices_16 <- c(634, 592, 3284)                # inputing the numbers for a slice of the pie chart
lbls_16 <- c("Black", "Other", "White")       # labelng the slices of the pie chart correctly
pct_16 <- round(slices_16/sum(slices_16)*100) # rounding the numbers and multiplying by 100
lbls_16 <- paste(lbls_16, pct_16) # add percents to labels 
lbls_16 <- paste(lbls_16,"%",sep="") # add % to labels 
pie(slices_16,labels = lbls_16, col=rainbow(length(lbls_16)), 
main="Percent Distribution of Racial Groups - 2016")


# Bar Chart
count(GSS2016, conclerg)          #how did people respond

GSS2016 %>%                       #plots a chart or mapping
  ...
  ggtitle("Confidence in Clergy - GSS 2016")+ # main title
  labs(x = "Confidence in Organized Religion")  # axis labels


GSS2016 %>%                    #similar but 2 different instructions for plot
  filter(!is.na(conclerg)) %>% # removing all NA's
...
unique(GSS2016$childs) # recheck the unique values to
summary(GSS2016$childs) # summary statistics of `childs'

# plotting histogram using ggplot2
ggplot(GSS2016, aes(x=childs)) +
  ...
  ggtitle("Number of Children in Household - 2016") + # main title
